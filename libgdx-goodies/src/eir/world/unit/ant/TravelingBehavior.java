...
public abstract class TravelingBehavior implements UnitBehavior <Ant>
{
	...
	protected void travelTo( final float delta, final Task task, final Ant ant, final SurfaceNavNode targetNode )
	{
		if(ant.nextNode == null)
		{
			...
			ant.velocity.set( ant.nextNode.getPoint() ).sub( ant.getBody().getAnchor() ).nor().scl( ant.getMaxSpeed() );
		...
		while(travelDistance > 0)
		{
			...
			if(travelDistance < 0)
			{
				ant.velocity.set( ant.nextNode.getPoint() ).sub( ant.anchor.getPoint() ).nor().scl( ant.speed );
				ant.angle = ant.velocity.angle();
				break;
			}
...
//			task.cancel();
		...
		ant.getBody().getAnchor().set(
				edge.getDirection() ).scl( ant.nodeOffset ).add( ant.anchor.getPoint() );
	}
